  ...
  return (
    <TabsTrigger
      ...
      className={cn(
        ...
        currentTab === value ? 'data-[state=active]:bg-brand data-[state=active]:text-brand-foreground' : 'hover:bg-background/60'
      )}
    >
      {children}
    </TabsTrigger>
    ...
    <header className="border-border/40 bg-background/95 supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 flex h-10 w-full border-b px-4 backdrop-blur">
      <div className="min-w-[200px] w-auto flex items-center">
        <a href={webuiPrefix} className="flex items-center gap-2">
          <ZapIcon className="size-4 text-brand" aria-hidden="true" />
          <span className="font-bold text-base md:inline-block">{SiteInfo.name}</span>
        </a>
        {webuiTitle && (
